...
import { currentParserIds, currentParsers, currentParsersGuis } from '~/state/parser/parser'
    ...
    <TabsTab v-show="sideBarAvailable" v-model="activeTab" :tabs="tabs">
      <TabsTabPane v-for="(tab, idx) in tabs" :key="tab.label" :label="tab.label" :value="tab.value">
        ...
        <h2 flex="~ center" gap2 text-lg font-bold>
          Parser Options
          <ParserOptions v-if="currentParsers[idx]!.options.configurable" :parser-id="currentParsers[idx]!.id"
            :index="idx" nav-button />
        </h2>
        <Suspense :timeout="0">
